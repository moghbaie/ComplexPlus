...
Networks <- R6Class("Networks",  list(
  ...
  # Disease similary graph saved from previous calculation
  diseaseSimilarityFile = "../Data/Layers/Disease/graph/DiseaseSimilarity_2018-08-17.gr",
  goaHumanMapping = "../Data/Mapping/goa/goa_human.txt",
  # Initial graph
  ...
  getDirectInteraction = function(){
    ...
    tbl_direct3 = data.frame(do.call("rbind",apply(tbl_direct2, 1, fillconfidenceScore)))
  ...
  unionProteinList = function(){
    ...
    protein2Gene <- add.vertices(protein2Gene, nv=length(self$proteinList[!self$proteinList %in% common_list$uniprotswissprot]),
                                 attr = list(name=self$proteinList[!self$proteinList %in% common_list$uniprotswissprot]))
  ...
  buildDiseaseSimilarityNet = function(){
    ## Reading disease similarity grapph
    DiseaseSimilarity = read.table(self$diseaseSimilarityFile, quote="\"", comment.char="")
    disease_similarity_graph = graph_from_data_frame(DiseaseSimilarity[c("V1", "V2")], directed = FALSE)
    uniqueDisease = data.frame(unique(unlist(DiseaseSimilarity)))
    ## Make mapping between protein and disease 
    ensembl = useMart("ensembl",dataset="hsapiens_gene_ensembl")
    disease_protein<- getBM(attributes=c("mim_morbid_accession" ,"uniprotswissprot"), mart = ensembl, filters = "mim_morbid_accession", values = uniqueDisease$unique.unlist.DiseaseSimilarity..)
    Disease2Protein <- data.frame(unique(disease_protein[as.character(disease_protein$mim_morbid_accession) %in% as.character(uniqueDisease$unique.unlist.DiseaseSimilarity..) & as.character(disease_protein$uniprotswissprot) %in% as.character(self$proteinList), ]))
    #colnames(Disease2Protein)
    g_Disease2Protein <- graph.empty(directed = F)
    out_Disease2Protein  <- unique(Disease2Protein$mim_morbid_accession) #stringsAsFactor = F in data frame
    in_Disease2Protein <- unique(Disease2Protein$uniprotswissprot) #stringsAsFactor = F in data frame
    g_Disease2Protein <- add.vertices(g_Disease2Protein,nv=length(out_Disease2Protein),attr=list(name=out_Disease2Protein),type=rep(FALSE,length(out_Disease2Protein)))
    g_Disease2Protein <- add.vertices(g_Disease2Protein,nv=length(in_Disease2Protein),attr=list(name=in_Disease2Protein),type=rep(TRUE,length(in_Disease2Protein)))
    edge.list.vec_Disease2Protein<- as.vector(t(as.matrix(data.frame(Disease2Protein))))
    g_Disease2Protein  <- add.edges(g_Disease2Protein ,edge.list.vec_Disease2Protein )
    extra_diseases <- as.character(unique(uniqueDisease$unique.unlist.DiseaseSimilarity..[!as.character(uniqueDisease$unique.unlist.DiseaseSimilarity..) %in% as.character(Disease2Protein$mim_morbid_accession)]))
    g_Disease2Protein  <- add.vertices(g_Disease2Protein,nv=length(extra_diseases),attr=list(name = extra_diseases),type=rep(FALSE,length(extra_diseases)))
    extra_proteins <- as.character(unique(self$proteinList[!as.character(self$proteinList) %in% as.character(Disease2Protein$uniprotswissprot)]))
    g_Disease2Protein  <- add.vertices(g_Disease2Protein, nv=length(extra_proteins),attr=list(name=extra_proteins),type=rep(TRUE,length(extra_proteins)))
    pr <- sort(unique(c(extra_proteins,in_Disease2Protein)))
    dis <- sort(unique(c(extra_diseases,out_Disease2Protein)))
    adj<- get.adjacency(g_Disease2Protein, sparse = TRUE) 
    M_Disease2Protein  <- adj[rownames(adj) %in% pr , colnames(adj) %in% dis]
    
    save(uniqueDisease, M_Disease2Protein , file=self$disease2ProteinBipartiteMapping)
    save(disease_similarity_graph, file = self$diseaseSimilarityGraph)
    self$diseaseList = uniqueDisease
    invisible(self)
  }, 
  buildGoBipartiteMappingNet = function(){
    # Import goa_human file 
    # Build mapping between GO and protein
    # Build categorical Go list
    H.sapiens = read.delim(self$complexPortalHS)
    
    goList = unique(unlist(apply(H.sapiens, 1, function(x) 
      substr(gsub("\\s*\\([^\\)]+\\)","",strsplit(as.character(x[["Go.Annotations"]]) ,"\\|")[[1]]),0,10)
      )))
    self$goList = goList[goList!="-"]
    
    goa_human <- read.delim(self$goaHumanMapping, header=FALSE)
    go_protein <- data.frame(unique(goa_human[,c("V5","V2","V9")]))
    colnames(go_protein) <- c("go_id","uniprot", "category")
    GO2Protein <- go_protein %>% filter(go_id %in% goList & uniprot %in% self$proteinList)
    
    g_GO2Protein <- graph.empty(directed = F)
    out_GO2Protein  <- unique(as.character(GO2Protein$go_id)) #stringsAsFactor = F in data frame
    in_GO2Protein <- unique(as.character(GO2Protein$uniprot)) #stringsAsFactor = F in data frame
    g_GO2Protein <- add.vertices(g_GO2Protein,nv=length(out_GO2Protein),attr=list(name=out_GO2Protein),type=rep(FALSE,length(out_GO2Protein)))
    g_GO2Protein <- add.vertices(g_GO2Protein,nv=length(in_GO2Protein),attr=list(name=in_GO2Protein),type=rep(TRUE,length(in_GO2Protein)))
    edge.list.vec_GO2Protein <- as.vector(t(as.matrix(data.frame(GO2Protein)[,-3])))
    g_GO2Protein  <- add.edges(g_GO2Protein,edge.list.vec_GO2Protein)
    E(g_GO2Protein)$category = as.character(data.frame(GO2Protein)[,3])
    
    extra_go <- as.character(unique(goList[!as.character(goList) %in% as.character(GO2Protein$go_id)]))
    g_GO2Protein  <- add.vertices(g_GO2Protein,nv=length(extra_go),attr=list(name = extra_go),type=rep(FALSE,length(extra_go)))
    
    extra_proteins <- as.character(unique(self$proteinList[!as.character(self$proteinList) %in% as.character(GO2Protein$uniprot)]))
    g_GO2Protein <- add.vertices(g_GO2Protein , nv=length(extra_proteins),attr=list(name=extra_proteins),type=rep(TRUE,length(extra_proteins)))
    
    pr <- sort(unique(c(extra_proteins,in_GO2Protein)))
    gl <- sort(unique(c(extra_go,out_GO2Protein)))
    adj2 <- get.adjacency(g_GO2Protein , sparse = TRUE) 
    M_GO2Protein  <- adj2[rownames(adj2) %in% gl , colnames(adj2) %in% pr]
    save(goList, M_GO2Protein ,g_GO2Protein, file = self$go2ProteinBipartiteMapping)
  }
  )
)
